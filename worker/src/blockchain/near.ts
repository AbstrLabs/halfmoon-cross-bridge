...
class NearBlockchain extends Blockchain {
  ...
  async sendTransaction(t: Uint8Array): Promise<void> {
    ...
    await provider.sendJsonRpc('broadcast_tx_async', [
  ...
  async verifyIncomingTransaction(
    request: RequestForVerify,
    fromToken: FromToken
  ): Promise<VerifyResult> {
    let txnOutcome = await this.getTransaction(
      request.from_txn_hash,
      request.from_addr
    );
    if (txnOutcome.status instanceof Object) {
      if (txnOutcome.status.Failure !== undefined) {
    ...
    // check from address
    if (txnOutcome.transaction.signer_id !== request.from_addr) {
      return {
        valid: false,
        invalidReason: 'transaction signer does not match',
      };
    }
    ...
    if (
      txnOutcome.transaction.actions[0].FunctionCall?.method_name !=
      'add_bridge_request'
    ) {
      return {
        valid: false,
    ...
    if (args.to_address != request.to_addr) {
      return {
        valid: false,
    ...
    if (receivedAtom !== request.from_amount_atom) {
      return {
        valid: false,
