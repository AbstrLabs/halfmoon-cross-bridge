import { VerifySuccessData } from "./types";
...
export class StateMachine {
    ...
    async verifySuccess(verifySuccessData: VerifySuccessData) {
        log.debug(`Request id ${this.id} verify success`);
        await this.client.query(sql.updateRequestCreatedToDoneVerify({ id: this.id, ...verifySuccessData }));
    }
